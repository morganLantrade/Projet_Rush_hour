...
class FenetreMenuPrincipale:
    ...
    def ouvrir_regles(self):
        self.menu_regles = PageRegles(master=self.root, app=self)
        self.canvas.pack_forget()
        self.menu_regles.lancer_regles()

    def ouvrir_choix_niv(self):
        self.menu_niveaux = PageNiveaux(master=self.root, app=self)
        self.canvas.pack_forget()
...
class PageNiveaux:
    def __init__(self, master=None, app=None):
        ...
        self.difficultes=["Facile","Normal","Difficile"]
        ...
        for niveau in Niveaux: 
            if cpt<3:
                 ...
                 boutton.config(command= lambda: self.load_niveau(cpt))
                 cpt+=1
